...
import org.fundacionjala.sfdc.pages.IStrategy;
...
public class ProductForm extends FormBase {
     ...
     */
    @Override
    public DetailBase newItemFromMap(Map<ProductFormField, String> formMapData) {
//        getStrategyMap(formMapData).forEach((key, value) -> value.performTask());
        formMapData.forEach((key, value) -> getStrategyMap(formMapData).get(key).performTask());
        clickSaveButton();
        return new ProductDetail();
    }
     ...
     * @return Map ProductFormField, IStrategy.
     */
    private Map<ProductFormField, IStrategy> getStrategyMap(Map<ProductFormField, String> formMap) {
        Map<ProductFormField, IStrategy> strategyMap = new HashMap<>();
